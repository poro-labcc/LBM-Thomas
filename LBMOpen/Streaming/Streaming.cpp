    ...
    for (int i = 0; i < Nx; i++) {
        for (int j = 0; j < Ny; j++) {
            ...
            for (int k = 0; k < K; k++) {
                ...
                double d = params.dist[index3D(ops_k[k], i, j)];

                if (!params.isSolid[index2d(ii, jj)]) {
                    // Streaming normal: pull from neighbor
                    ...
                    if (d < 0.5) {
                        ...
                        params.f_temp[index3D(k, i, j)] =
                            2.0 * d * params.f[index3D(ops_k[k], i, j)] +
                            (1.0 - 2.0 * d) * params.f[index3D(ops_k[k], i + params.cx[k], j + params.cy[k])];
                    } else {
                        //std::cout << "To aqui" << std::endl;
                        params.f_temp[index3D(k, i, j)] =
                            (1.0 / (2.0 * d)) * params.f[index3D(ops_k[k], i, j)] +
